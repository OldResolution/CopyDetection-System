...
class HybridDataStore:
        ...
        try:
            ...
            cursor.execute('''
                ...
                (doc_id, filename, title, author, subject, keywords, creator, producer,
                 creation_date, modification_date, page_count, file_size_bytes,
    ...
    def get_all_documents(self, limit: int = None) -> List[Dict]:
        ...
        query = '''
            SELECT doc_id, filename, title, author, page_count, 
                   char_count, word_count, chunk_count, indexed_at
            FROM documents
...
if __name__ == "__main__":
    ...
    docs = store.get_all_documents(limit=5)
    for doc in docs:
        print(f"  - {doc['title'] or doc['filename']} by {doc['author'] or 'Unknown'}")
    
    print("\n🔍 Example: Semantic search for 'ring of power'")
    results = store.semantic_search("ring of power", n_results=3)
    for r in results:
        print(f"  [{r['similarity']:.3f}] {r['title'] or r['filename']}")
